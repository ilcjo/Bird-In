...
export const CardsLand = React.memo(({ foto, name, index, id }) => {
  ...
  const memoizedDispatch = React.useCallback(
    (selectOption) => {
      dispatch(sendParameterP(selectOption));
    },
    [dispatch]
  );
  ...
  const handleDetailClick = () => {
    ...
    const selectOption = { zona: [{ id: id }] };
    memoizedDispatch(selectOption);
    // Guardar la información del ave seleccionada en localStorage
    localStorage.setItem('selectedRegister', JSON.stringify(selectOption));
    setTimeout(() => {
  ...
  return (
    <Card sx={{
      minWidth: { xs: 380, lg: 500 },
      maxWidth: { xs: 380, lg: 500 },
      minHeight: 330,
      maxHeight: 330,
      position: 'relative',
      borderRadius: '6px',
