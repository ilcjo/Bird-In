...
export const createRegistro = (formData) => {
  return async (dispatch) => {
    try {
      const response = await axios.post('peces/create', formData)
...
export const actualizarRegistro = (info) => {
  return async (dispatch) => {
    try {
      const response = await axios.put('peces/update', info)
...
export const getInfoForUpdate = (id) => {
  return async (dispatch) => {
    try {
      const response = await axios(`peces/get_update?id=${id}`)
...
export const getInfoForUpdateName = (name) => {
  return async (dispatch) => {
    try {
      const response = await axios(`peces/get_update_name?name=${name}`)
      const registro = response.data
      dispatch(getRegistro(registro))
      // Maneja la respuesta del servidor (puede ser un mensaje de éxito o error)
      console.log('Respuesta del servidor:');
...
export const duplicateNameCheck = (name) => {
  return async (dispatch) => {
    try {
      const response = await axios(`peces/duplicados?name=${name}`)
      const registro = response.data
      // Maneja la respuesta del servidor (puede ser un mensaje de éxito o error)
    } catch (error) {
      // Maneja los errores de la solicitud
...
export const deleteRegistro = (idN) => {
  ...
  return async (dispatch) => {
    try {
      const response = await axios.delete(`/peces/borrar_registro?id=${idN}`);
...
export const getExcel = () => async (dispatch) => {
  try {
    const response = await axios.get('/peces/descargar-excel', {
    ...
    link.href = url;
    link.setAttribute('download', 'peces.xlsx'); // nombre del archivo
