...
export const getOptionsDataPe = () => {
  return async (dispatch) => {
    try {
      // console.log('llegue')
      const response = await axios('peces/opciones')
...
export const fetchNewOptions = (selectedOptions) => {
  return async (dispatch) => {
    try {
      ...
      const response = await axios(`/peces/nuevasOpciones?${parameter}`);
      const data = response.data
...
export const clasesFamilia = (idfamilia) => {
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.get(`peces/clases?familiaID=${idfamilia}`);
...
export const clasesGrupo = (idgrupo) => {
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.get(`peces/clases?grupoID=${idgrupo}`);
