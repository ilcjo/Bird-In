...
export const getInfoRegistro = () => {
    return async (dispatch) => {
        try {
            const response = await axios('peces/filtros')
...
export const loadMoreData = (currentPage, parameters) => {
    return async (dispatch) => {
        try {
            const perPages = 18
            const response = await axios(`peces/filtros?${parameters}&page=${currentPage}&perPage=${perPages}`);
            const data = response.data.registrosFiltrados;
...
export const counting = () => {
    return async (dispatch) => {
        try {
            const response = await axios('peces/contando');
