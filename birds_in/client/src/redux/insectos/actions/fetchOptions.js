...
export const getOptionsDataI = () => {
  return async (dispatch) => {
    try {
      // console.log('llegue')
      const response = await axios('insectos/opciones')
...
export const fetchNewOptions = (selectedOptions) => {
  return async (dispatch) => {
    try {
      ...
      const response = await axios(`/insectos/nuevasOpciones?${parameter}`);
      const data = response.data
...
export const clasesFamilia = (idfamilia) => {
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.get(`insectos/clases?familiaID=${idfamilia}`);
...
export const clasesGrupo = (idgrupo) => {
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.get(`insectos/clases?grupoID=${idgrupo}`);
