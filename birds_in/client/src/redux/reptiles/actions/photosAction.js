...
export const saveImageFtp = (formData) => {
    return async (dispatch) => {
      try {
        const response = await axios.post('reptiles/upload_image', formData, {
          headers: {
...
export const UpdateDestacada = (formData) => {
    return async (dispatch) => {
      try {
        const response = await axios.post('reptiles/upload_destacada', formData, {
          headers: {
            'Content-Type': 'multipart/form-data', // Asegúrate de establecer el tipo de contenido correcto
          },
        });
  ...
  export const sendPhotosDelete = (ids, urls) => {
    return async (dispatch) => {
        try {
            ...
            const response = await axios.delete('/reptiles/borrar_fotos', requestData)
...
export const sendCoverPhoto = (id, idAves) => {
    ...
    return async (dispatch) => {
        try {
            ...
            const response = axios.put('/reptiles/foto_portada', requestData )
            const data = response.data
            return data
        } catch (error) {
            console.error("Error al obtener los datos:", error)
