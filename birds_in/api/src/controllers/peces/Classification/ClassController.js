const { Peces, Familias_peces, Grupos_peces } = require("../../../config/db/db");

const createFamilias = async (familia) => {
    try {
        if (familia) {
            await Familias_peces.create({
                nombre: familia,
...
const updateFamilias = async (nombreF, idFamilia) => {
    try {
        const existingFamilia = await Familias_peces.findOne({
            where: {
...
const borrarFamilias = async (idF) => {
    try {
        // Buscar todas las Peces que tienen el ID de la familia a cambiar
        const RegistrosConIdFamilia = await Peces.findAll({
            where: {
                familias_id_familia: idF,
            },
        });
        // Cambiar el ID de familia solo en las Peces encontradas
        await Peces.update(
            {
        ...
        await Familias_peces.destroy({
            where: {
                id_familia: idF,
            },
        });

        return `Se actualizaron ${RegistrosConIdFamilia.length} Peces al nuevo ID de familia  y Grupo a not specified.`;
    } catch (error) {
        console.error('Error:', error);
        throw error;
    }
...
const updateGrupo = async (nombreG, idGrupo) => {
    try {
        const existingGrupos = await Grupos_peces.findOne({
            where: {
                id_grupo: idGrupo,
            },
        })
...
const borrarGrupos = async (idG) => {
    try {
        // Buscar todas las Peces que tienen el ID de la familia a cambiar
        const RegistrosConIdGrupos = await Peces.findAll({
            where: {
                grupos_id_grupo: idG,
            },
        });
        ...
        await Grupos_peces.destroy({
            where: {
                id_grupo: idG,
            },
        });

        return `Se actualizaron ${RegistrosConIdGrupos.length} Peces al nuevo ID de Grupo  y Familia a not specified.`;
    } catch (error) {
        console.error('Error:', error);
        throw error;
    }
