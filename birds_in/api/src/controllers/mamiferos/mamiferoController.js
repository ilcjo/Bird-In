...
const buildIncludeArray = () => {
    return [
        ...
        },
        {
            ...
            attributes: ['url_mamifero', 'destacada']
        },
...
const filterOptions = async (grupo, familia, pais, nombreIngles, nombreCientifico, zonas) => {
    ...
    allResults.registrosFiltrados.forEach(registro => {
        registro.zonasMamiferos.forEach(zona => zonasSet.add(JSON.stringify({
    ...
    newOptions.zonas = Array.from(zonasSet).map(zona =>
        JSON.parse(zona));
    const nombresCientificos = [...new Set(allResults.registrosFiltrados.map(registro => ({ id: registro.id_mamifero, nombre: registro.dataValues.nombre_cientifico })))];
    newOptions.nCientifico = nombresCientificos;
    const nombresIngles = [...new Set(allResults.registrosFiltrados.map(registro => ({ id: registro.id_mamifero, nombre: registro.dataValues.nombre_ingles })))];
    newOptions.nIngles = nombresIngles;
    // const listaZona = [...new Set(allResults.map(registro => ({ id: registro.id_mamifero, nombre: registro.dataValues.zonas })))];
...
const filterOptionsPaisZonas = async (familia,
    ...
    if (zonas || pais) {
        ...
        allResults.registrosFiltrados = allResults.registrosFiltrados.filter(registro => {
            ...
            const meetsZonasCriteria = !zonas || registro.zonasMamiferos.some(zona => zonas.includes(zona.dataValues.id_zona));
