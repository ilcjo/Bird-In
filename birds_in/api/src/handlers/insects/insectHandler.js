...
const getFilterInfo = async (req, res) => {
   ...
   try {
      const allData = await fetchFilterInsect(familia, grupo, nombreCientifico, nombreIngles, pais, zonas, page, perPage)
      if (allData.length === 0) {
...
const createInsect = async (req, res) => {
   ...
   try {
      ...
      const successCreate = await sendAndCreateInsect(
         grupo,
         familia,
         pais,
         zona,
...
const updateInfoRegister = async (req, res) => {
   ...
   try {
      const succesUpdate = await sendAndUpdateInsect(
         grupo,
         familia,
         pais,
         zona,
...
const deletePhotos = async (req, res) => {
   ...
   try {
      const deletedFtp = await deletePhotoFromFTPInsectos(urls);
