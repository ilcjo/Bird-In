...
const { sendAndCreateFish, fetchFilterFish, fetchOptions, filterOptionsPaisZonas, findDataById, findDataByName, sendAndUpdateFish, setDbCover, getContadores, findNameDuplicate, findAllEnglishNames, getClassGrupoFamilia, findGroupNameDuplicate, findFamilyNameDuplicate, findPhotosId, deleteRegistroDb, saveDbPhotoOrder, filterOptions } = require('../../controllers/peces/pecesController');

const getFilterInfo = async (req, res) => {

   const { familia, grupo, nombreCientifico, nombreIngles, pais, zonas, page, perPage } = req.query;
   try {
      const allData = await fetchFilterFish(familia, grupo, nombreCientifico, nombreIngles, pais, zonas, page, perPage)
      if (allData.length === 0) {
...
const createFish = async (req, res) => {
   ...
   try {
      ...
      const successCreate = await sendAndCreateFish(
         grupo,
         familia,
         pais,
         zona,
...
const uploadImageftp = async (req, res) => {
   try {
      ...
      for (const image of images) {
         ...
         const imageUrl = `https://lasavesquepasaronpormisojos.com/generalimag/peces/${remoteFileName}`;
...
const deletePhotos = async (req, res) => {
   ...
   try {
      const deletedFtp = await deletePhotoFromFTPpeces(urls);
...
//       res.setHeader('Content-Disposition', 'attachment; filename=peces.xlsx');
